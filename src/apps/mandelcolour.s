...
YMIN = $fed8  ; -1.25        11111110 11000000
YMAX = $0140  ;  1.25        00000001 01000000
YSTEP = $0002 ;  0.03125     00000000 00001000
XMIN = $ffb0  ; -1.546875    11111110 01110100
XMAX = $0090  ;  0.5625      00000000 10010000
XSTEP = $0001 ;  0.015625    00000000 00000100
...
.)
	...
	; Check aa+bb against 4
	clc
	lda zp_aa : adc zp_bb
	lda zp_aa+1 : adc zp_bb+1
	cmp #4 : bcs iterloopendearly
...
colours:
	.byte 15,15,15,15,15,15,15,14,13,12,15,11,14,10,13,9,12,15,11,14,10,13,9,12,7,3,6,2,5,1,4,7,3,6,2,5,1,4,7,3,6,2,5,1,4,7,3,6,2,5,1,4
numcolours = *-colours
...
.(
	dec
	tax : lda colours,x
	cmp zp_lastcolour : bne changecolour
	rts
changecolour:
	sta 0
	eor zp_lastcolour : and #8 : beq withoutreset
	lda #27 : jsr printchar
	lda #'[' : jsr printchar
	lda #'0' : jsr printchar
	lda #';' : jsr printchar
	lda #8 : cmp 0 : rol : and #1 : adc #1 : ora #48 : jsr printchar
	lda #';' : jsr printchar
tailend:
	lda #'3' : jsr printchar
	lda 0 : sta zp_lastcolour
	and #7 : ora #48 : jsr printchar
	lda #'m' : jsr printchar
	rts
withoutreset:
	lda #27 : jsr printchar
